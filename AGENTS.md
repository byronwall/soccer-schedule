# AGENTS.md - Soccer Coach Companion
...
- Treat this repository as Soccer Coach Companion, not as a reusable starter. Remove visible starter placeholders when touching their surrounding feature.
...
- Reuse the one running server advertised by repo-root `live-server-details.json`. Never start a second server for the same checkout: all wrappers would hot-reload the same files and mutate the same development data. The wrapper refuses duplicate starts and clears its advertised URL when it exits.
...
- Treat `app/data` as disposable local development state until the project is deployed. Manual browser checks may mutate it; do not add fixture infrastructure solely to protect it.
...
- Current soccer-domain rules: `docs/soccer-domain-invariants.md`
...
- Use Browser or Playwright MCP for task-driven manual verification. Do not add Playwright Test, an E2E suite, or CI solely for this personal application.
- Include a mobile viewport when a change affects game-day tracking or another mobile-sensitive workflow. Use `docs/manual-browser-verification.md` to choose checks; do not run the entire checklist for every change.
...
- The application retains an inherited file-backed SaaS scaffold for prototype auth, Stripe, email, analytics, and admin JSON endpoints. Keep inactive integrations generic; move Soccer Coach Companion products, prices, and copy into env/config wrappers before enabling them.
